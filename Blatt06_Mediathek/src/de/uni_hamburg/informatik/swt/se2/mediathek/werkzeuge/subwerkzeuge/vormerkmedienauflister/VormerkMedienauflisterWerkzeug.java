...
public class VormerkMedienauflisterWerkzeug extends ObservableSubWerkzeug
{
    ...
    private void setzeAnzuzeigendeMedien()
    {
        ...
        for (Medium medium : medienListe)
        {
            ...
            {
            	vormerker1 = _verleihService.getVormerkkarteFuer(medium).getVormerker(1);
            	
            	if(_verleihService.getVormerkkarteFuer(medium).getAlleVormerker().size() > 1)
                {
                	vormerker2 = _verleihService.getVormerkkarteFuer(medium).getVormerker(2);
                }
                
                if(_verleihService.getVormerkkarteFuer(medium).getAlleVormerker().size() == 3)
                {
                	vormerker3 = _verleihService.getVormerkkarteFuer(medium).getVormerker(3);
                }
            }
